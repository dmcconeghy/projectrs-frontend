...
function Podcasts(){
    ...
    const podcastCards = podcasts.map(episode => 
        <PodcastCard key={episode.podcast_id} episode={episode} />);
    ...
    return (
        <>
            <div className="podcastsList">
                ...
                <p>Links: <a href={`/podcasts/?title=${title}&content=${content}&page=${page - 1 < 1 ? 1 : page - 1}&limit=${limit}`}>previous</a> |
                 <a href={`/podcasts/?title=${title}&content=${content}&page=${page + 1 > pages ? page + 1: page + 1}&limit=${limit}`}>next</a></p>
                {podcastCards}
            </div>
        </>
    )
